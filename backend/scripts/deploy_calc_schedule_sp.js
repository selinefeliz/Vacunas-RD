...
async function deploySp() {
    try {
        ...
        const spPath = path.join(__dirname, '../../database/programmability/usp_CalcularEsquemaVacunacionNino.sql');
        const spContent = fs.readFileSync(spPath, 'utf8');
        const cleanContent = spContent.replace(/\bGO\b/gi, '');

        await pool.query(cleanContent);
        console.log('Successfully deployed usp_CalcularEsquemaVacunacionNino');
        process.exit(0);
    } catch (err) {
